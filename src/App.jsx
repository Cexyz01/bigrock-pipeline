      ...
      <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        {(view === 'storyboard' || view === 'timeline' || view === 'timeline2' || view === 'pack' || view === 'gantt' || view === 'review') ? (
          ...
          <div style={{ padding: contentPadding, ...(isMobile ? {} : { maxWidth: 1400 }), width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
            ...
            {view === 'shots' && <ShotTrackerPage shots={shots} assets={assets} tasks={tasks} user={user} canEditShots={myPerms.can_manage_shots} onUpdateShot={handleUpdateShot} onReorderShots={handleReorderShots} onCreateShot={handleCreateShot} onDeleteShot={handleDeleteShot} onUploadReference={handleUploadReference} onUploadOutput={handleUploadOutput} onCreateAsset={handleCreateAsset} onUpdateAsset={handleUpdateAsset} onDeleteAsset={handleDeleteAsset} onReorderAssets={handleReorderAssets} onUploadAssetReference={handleUploadAssetReference} onUploadAssetOutput={handleUploadAssetOutput} addToast={addToast} requestConfirm={requestConfirm} onGoToShotTasks={(shotId, deptId) => { setDeepLink({ type: 'shotFilter', id: shotId, dept: deptId || null }); setView('tasks') }} onGoToAssetTasks={(assetId, deptId) => { setDeepLink({ type: 'assetFilter', id: assetId, dept: deptId || null }); setView('tasks') }} />}
            ...
            {view === 'office' && hasPermission(user, 'access_admin_console') && <OfficePage tasks={tasks} profiles={profiles} projectMembers={projectMembers} user={user} currentProject={currentProject} onlineUsers={onlineUsers} onNavigate={handleNavigate} />}
